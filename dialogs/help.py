...
class RenderMethodHelpDialog(HelpDialog):
    ...
    <p>Two further dials pool that weighting before it is applied, both off by default and both costing a second pass over the stack. Weight coherence smooths each frame's share of the blend across the frame, guided by that frame, so a region no frame resolves stops being stitched from slices at opposite ends of the stack while genuinely sharp detail keeps its edges. Reach is the whole risk: it assumes depth varies smoothly out to the radius set, which pays on a deep capture of a real surface and costs where the correct frame changes every few pixels. Slice coherence pools along the other axis, averaging each pixel's weight across neighbouring frames, which divides the grain by roughly the square root of the count at no cost in spatial resolution. Set it from the capture's sampling rather than its content &mdash; about half the width of the focus peak, so a few slices on a densely oversampled sweep and zero on one that steps a full depth of field per frame &mdash; and leave it at zero on an unregistered stack, since it averages frames that must agree on where each pixel is. Runs on CPU or GPU.</p>
