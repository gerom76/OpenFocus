...
class RenderMethodHelpDialog(HelpDialog):
    ...
    <p>Depth Map (Max)<br/>
    Classic hard depth-map fusion: a local Laplacian-energy focus measure is computed per frame, and each pixel is taken whole from the frame where that measure is highest. The decision is an order-independent per-pixel argmax, so colour and noise stay clean within a slice and never blend across sources. The kernel slider sets the window the focus measure is pooled over - larger is steadier on noise, smaller follows finer detail. Depth coherence sets how readily a pixel's own decision is given up as unfounded, with the depth of every pixel given up on interpolated from the ones that were not: over a region no frame ever resolves every frame measures the same and the winner is decided by noise, which without this tears such a region into a mosaic of hard-edged patches taken from opposite ends of the stack. Edge coherence is the complement, and reaches what that cannot: because the fill is gated on how much the measure had to say, it declines to act over the regions the measure found merely adequate, and those are where the depth is noisiest without being obviously wrong. Filtering the finished depth map against the all-in-focus picture averages it within a surface and leaves it stepping at an occlusion, which a plain blur of the same reach cannot do without rounding the step off too. Keep the radius at or under the pooling window &mdash; past that it starts reaching over real depth steps. Runs on CPU or GPU.</p>

    <p>Depth Map (Average)<br/>
    Contrast-weighted average: frames are blended in proportion to the same local focus measure rather than hard-selected. Sharp detail is still dominated by the frame that holds it, but a flat region - where every frame is equally focused - collapses to the plain mean, recovering the stack's multi-frame signal-to-noise (a free sqrt(N) noise reduction). Use it on stacks with large smooth areas where Max would chase sensor noise. The kernel slider sets the focus-measure window, and Selectivity sets how sharply the blend leans on the frame that holds the detail &mdash; without it a deep stack's defocused majority outvotes the sharp frame by sheer count and the result is veiled.</p>

    <p>Weight coherence pools that weighting across the frame before it is applied, off by default and costing a second pass over the stack. It smooths each frame's share of the blend, guided by that frame, so a region no frame resolves stops being stitched from slices at opposite ends of the stack while genuinely sharp detail keeps its edges. Reach is the whole risk: it assumes depth varies smoothly out to the radius set, which pays on a deep capture of a real surface and costs where the correct frame changes every few pixels. Runs on CPU or GPU.</p>

    <p>Slice coherence (both depth-map modes)<br/>
    The other axis, and the one dial the two modes share. Averaging along the stack divides the grain by roughly the square root of the number of frames averaged at no cost in spatial resolution, which no spatial setting can offer &mdash; and on Max it is the only way to get it at all, since every other dial there moves the depth map while the pixel still comes from one frame whole. Average pools each pixel's weight across neighbouring frames; Max renders each pixel from a band of frames around its own depth instead of from the single winner. Set it from the capture's sampling rather than its content &mdash; about half the width of the focus peak, so a few slices on a densely oversampled sweep and zero on one that steps a full depth of field per frame, where the neighbours are the worst frames that resolve the pixel at all. Leave it at zero on an unregistered stack, since it averages frames that must agree on where each pixel is. Runs on CPU or GPU.</p>
