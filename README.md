...
> [!NOTE]
> 🎉 **2026.08.08**: **Pyramid: a seventh control, a tighter tuning loop, and one default.** **Grain estimate** — the share of each band the noise gate reads its level off — was reachable only by editing the module and is now a control like the rest: exposed in the right panel, saved, inherited by batch jobs, written into filenames and recorded in the render's metadata. **Base band** moves 3 → 8, worth +0.86 dB on the defocused-veil fixture and +0.42 on a deep stack with nothing measurably worse. The tuning loop gained two guards it should always have had: it now re-checks the deep-stack and veil fixtures the pyramid rework was built against, and it **re-measures its winner on all 333 frames** before writing anything, because recovered contrast rises with frame count and the bracket it is held to cannot bite on the subsampled stack the search runs on. That second guard immediately paid for itself — Selectivity 16 wins on every ground-truth fixture and on every subsampled reading, and on the full capture it recovers 1.233 of Helicon's contrast against 1.112, out of bracket, by sharpening grain rather than detail. It was reverted. New page: [pyramid_reference_report.html](reports/pyramid_reference_report.html), the only report here measured against a real reference rather than a generated ground truth. See item 19 in [ALGORITHM_IMPROVEMENTS.md](docs/ALGORITHM_IMPROVEMENTS.md).

> 🎉 **2026.08.04**: **Depth Map on the GPU, and lighter on the CPU.** The last fusion method without a device path has one: the focus measure, the per-pixel select, the contrast-weighted blend and halo suppression all run batched on CUDA/MPS with automatic CPU fallback — **7.9x** faster in *Max* and **12.8x** in *Average* on a 12-frame 12 MP stack, and **6.4x** with a halo radius set. Frames stream through in chunks sized from the VRAM the card reports free rather than a fixed batch, and the chunk halves itself and retries if the allocator still runs out, so the method scales from a laptop GPU to a workstation without a setting. The halo element is reproduced span by span from OpenCV's ellipse, so a radius means the same thing on both paths — the device dilation matches `cv2.dilate` exactly, not approximately. The CPU path keeps the same pixels to the bit while giving back **1.1 GB** of peak memory on that stack: *Max* now carries a one-byte-per-pixel decision map instead of a float32 accumulator and gathers the result at the stack's own depth, *Average* folds its accumulators in place, and the measurement pool is capped by free memory as well as by thread count — eight 60 MP frames in flight are 5.7 GB of float32 nobody had budgeted for.
...
- **Pyramid**: Laplacian-pyramid fusion. Band by band, each frame is weighted by how far its local energy falls behind the best on offer, so a sharp frame wins outright while frames that tie — a background no frame resolves — are averaged rather than picked between, and the result is held inside the range its own frames span. Seven tuning controls, all defaulted to their measured best. A strong general-purpose default.
- **Depth Map**: Per-pixel depth-map fusion from a local Laplacian focus measure, in two modes. *Max* takes each pixel whole from the sharpest frame (an order-independent hard select that keeps colour and noise clean within a slice); *Average* blends frames by their focus measure, so flat regions collapse to the mean and recover the stack's multi-frame SNR (√N noise reduction) while detail still follows the sharpest frame. Both run on GPU (CUDA/MPS) with automatic CPU fallback, and both take a halo-suppression radius.
