...
> [!NOTE]
> 🎉 **2026.07.26**: **Metadata on saved results.** A fused JPEG or PNG now inherits the **EXIF** block of the first source frame of the render — camera, lens and exposure survive the stack — and carries an **XMP** `OpenFocus` group with the program version, the render date and how long the render took. Both are spliced into the encoded file, so pixels are never recompressed and 16-bit PNG stays 16-bit. Applies to single saves, drag-out and batch output.
...
- **Metadata Passthrough**: A saved JPG/PNG result keeps the EXIF of the first source frame and records the OpenFocus version, render date and render duration in its own XMP group — added without recompressing the image.
