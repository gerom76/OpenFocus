...
# the detail; this decides how far that decision is allowed to be pooled across
# the frame before it is applied. See the block above DEFAULT_COHERENCE_RADIUS
# in fusion_methods/depthmap.py for what it was measured to fix.
#
# Defaults to off, which is what every render did while it was reachable only
# from the test harness, so exposing it changes no existing result.
#
# The filter is edge-aware, so it costs nothing over a region the blend already
# agreed with itself, and a great deal over one whose correct frame changes every
# few pixels: measured best at 16 on a 333-frame capture of a real surface, and
# best at 0-4 on the banded synthetic stacks in tests/fusion_scenarios.py, which
# step depth every 27 px. The ceiling sits past the measured optimum rather than
# past the point of harm, because where the harm starts is a property of the
# scene and not of the number.
...
# the hard select renders its depth map through a tent that wide instead of
# copying from the single winner. Same number either way, because what sets it
# is how densely the capture sampled its own depth of field and not what the
# method does with that. See DEFAULT_SLICE_RADIUS and the block above it in
# fusion_methods/depthmap.py, and BLEND_WIDTH_FLOOR for what MODE_MAX is opting
# out of when it is turned up.
#
# The right value follows the capture's sampling rather than its content - about
# half the half-maximum width of the focus curve, so 4-5 on a sweep that
# oversamples its depth of field sevenfold, and 0 on one that steps a full depth
# of field per frame, where the neighbouring slices are the ones that resolve the
# pixel worst. Overshooting is not subtle: at 8 slices on the stack it was
# measured on, every fifth of the frame got worse at once. The ceiling is
# generous against that measurement because a denser sweep wants proportionally
# more, not because 16 is safe.
#
