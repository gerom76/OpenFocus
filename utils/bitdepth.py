...
def supports_16bit(extension: str) -> bool:
    ...
    PNG, TIFF, JPEG XL and DNG can; JPEG, BMP and WebP cannot, so writing 16-bit
    data to them has to be narrowed first rather than silently mangled by the
    encoder.
    """
    ext = extension.lower()
    if not ext.startswith("."):
        ext = "." + ext
    return ext in (".png", ".tif", ".tiff", ".jxl", ".dng")
