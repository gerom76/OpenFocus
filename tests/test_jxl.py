"""Saving results as JPEG XL.

JPEG XL is the one output container OpenCV does not encode, so it goes through
utils.jxl and libjxl instead. Three promises are tested here:
...
3. A build without the encoder offers no JPEG XL anywhere rather than failing a
   save (`TestWithoutTheEncoder`).
