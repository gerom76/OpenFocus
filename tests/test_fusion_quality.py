...
def test_output_size_for_awkward_dimensions(method, fixture):
    ...
    Every method must hand back the geometry it was given. DCT used to crop to
    a multiple of its block size and DTCWT to pad an odd edge up to even, which
    silently misaligns any caller that assumes otherwise - the tiled renderer
    accumulates fused tiles at fixed offsets.
    """
    ...
    assert out.shape == stack[0].shape, (
        f"{method.label} turned {height}x{width} into {out.shape[0]}x{out.shape[1]}")
